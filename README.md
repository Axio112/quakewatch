...
```powershell
docker pull your-dockerhub-user/quakewatch:0.1.0
```

**Run the container**
```powershell
docker run -d --rm --name quakewatch -p 5000:5000 your-dockerhub-user/quakewatch:0.1.0
```
...
How CI/CD works
---------------
1) **Build:** Jenkins builds a new image tag `your-dockerhub-user/quakewatch:0.1.<BUILD_NUMBER>` from the repo root Dockerfile.
...
3) **Deploy:** Jenkins runs:
   ...
   helm upgrade --install quakewatch charts/quakewatch \
     --set image.repository=your-dockerhub-user/quakewatch \
...
# Expected: your-dockerhub-user/quakewatch:0.1.<BUILD_NUMBER>
```
...
Versioning
----------
- Images: `your-dockerhub-user/quakewatch:0.1.<BUILD_NUMBER>` (built by Jenkins).
